...
class PhieuXacNhanScreen extends StatefulWidget {
  final Store store;

  PhieuXacNhanScreen({required this.store});

  @override
  _PhieuXacNhanScreenState createState() => _PhieuXacNhanScreenState();
}

class _PhieuXacNhanScreenState extends State<PhieuXacNhanScreen> {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: Scaffold(
          ...
          appBar: AppBar(
            title: const Text('Phiếu xác nhận'),
            leading: IconButton(
                      ...
                      children: [
                        Container(
                          ...
                          child: const SizedBox(
                            ...
                            child: Center(
                              child: Text(
                                'Chụp 1 tấm người đại diện kí và 1 tấm PXN',
  ...
  Future<List<String>> captureImageFee() async {
    setState(() {
      _isLoading = true;
    });
  ...
  Future<void> handleCaptureImageFee() async {
    setState(() {
      _isLoading = true;
    });
    List<String> imagePath = localFilePaths;
    if (imagePath != null && _countPhoto >= 2) {
      await uploadImage(
          widget.store.id, imagePath, "fee_info", latitude, longitude);
